 ...
 * @returns {Object}
 */
export default async function LodexJoinQuery(this: any, data: any, feed: any) {
    if (this.isLast()) {
        return feed.close();
    }
    const { ezs } = this;
    ...
    let hitsTotal = 0;
    const results = {};
    await aggregateCursor.forEach((row: any) => {
        hitsTotal += 1;
        _.get(row, 'items', []).forEach((item: any) => {
            if (item !== matchValue) {
