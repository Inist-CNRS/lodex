 ...
 * @returns {string}
 */
function formatOutput(this: any, data: any, feed: any) {
    ...
    const keys = extracts.filter((x: any) => x);

    // @ts-expect-error TS(2304): Cannot find name 'JSON'.
    const json = (d: any) => JSON.stringify(d, null, indent ? '    ' : null);
    ...
    if (this.isFirst() && !this.isLast()) {
        const values = keys.map((p: any) => get(data, p));
        feed.write('{');
        if (keys.length > 0) {
            let check = false;
    ...
    keys.forEach((p: any) => unset(data, p));
