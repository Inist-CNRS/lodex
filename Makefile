...
build: ## Build the docker image localy
	docker build -t inistcnrs/lodex:12.26.2 --build-arg http_proxy --build-arg https_proxy .
publish: build  ## publish version to docker hub
	docker push inistcnrs/lodex:12.26.2
...
ifeq "$(DISABLE_E2E_TESTS)" "true"
	...
	npx cypress install
	./bin/wait-for -t 30 localhost:3000 -- npx cypress run --browser chrome --spec "cypress/integration/publication.spec.js" || (\
