...
LIMIT 100`;
    var options = {
        catalogueEndpoints: [
            ...
            {
                endpoint: 'http://www.rechercheisidore.fr/sparql',
                title: 'ISIDORE',
            },
            {
    ...
    fetch('/triplestore/sparql/examples.json').then((response) => {
        response.json().then(function (elems) {
            initExamples(elems, yasgui);
        });
    });
}, 1000);
...
function initExamples(examplesInJson, yasgui) {
    ...
    executeBt.addEventListener('click', function () {
        ...
        if (selected.description !== undefined) {
            query =
                '# ' +
                wordwrap(selected.description, 80, '# ') +
                '\n\n' +
                selected.query;
        }
    ...
    document.addEventListener('click', function (event) {
        ...
        if (event.target.closest('.dropdown-open') === null && closeDropdowns())
            return null;
...
function shouldShowExample(elem, search, selectedTags) {
    ...
    if (
        ...
        (elem.query === undefined && elem.title !== '---')
    ) {
        return false;
    }
    ...
    const matchDescription =
        elem.description !== undefined && contains(elem.description, search);
    const matchTitleOrDescription = matchTitle || matchDescription;

    //filtre uniquement sur titre et description
    if (selectedTags.length === 0 && matchTitleOrDescription) {
        return true;
    }
    ...
    return (
        elem.tags !== undefined &&
        (search === '' || matchTitleOrDescription) &&
        elem.tags.some(isSelectedTag)
    );
}
...
function getUrlArgs(url) {
    ...
    const cleaned = keysValues.map((keyValue) =>
        keyValue.map((elem) => decodeURIComponent(elem.replace(/\+/g, ' '))),
    );
